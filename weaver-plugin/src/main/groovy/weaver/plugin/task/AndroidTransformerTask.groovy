...
public class AndroidTransformerTask extends Transform {
  ...
  void transform(TransformInvocation transformInvocation)
    ...
    try {
      def doLast = { Set<CtClass> allClasses ->
  ...
  TransformBundle createTransformBundle(TransformInvocation transformInvocation) {
    ...
    WeaverClassPool pool = createClassPool(rootClassLoader, transformInvocation.inputs,
   ...
   */
  private
  static WeaverClassPool createClassPool(URLClassLoader classLoader,
  ...
  private File getOutputFile(TransformOutputProvider outputProvider) {
    return outputProvider.getContentLocation('weaver', getInputTypes(), getScopes(),
        Format.DIRECTORY)
  }
   ...
   * @return
   */
  private copyResourceFiles(Collection<TransformInput> inputs,
